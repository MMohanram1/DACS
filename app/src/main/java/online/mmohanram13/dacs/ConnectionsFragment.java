...
public abstract class ConnectionsFragment extends Fragment {
    ...
    private final PayloadCallback mPayloadCallback =
            new PayloadCallback() {
                ...
                public void onPayloadReceived(String endpointId, Payload payload) {
                    ...
                    /*test=*/onReceive(mEstablishedConnections.get(endpointId), payload);
                }
  ...
  /**Returns whether a device has already been connected or not**/
  protected boolean isConnected(Endpoint endpoint){
      if (getConnectedEndpoints().contains(endpoint)){
          return true;
      }
      return false;
  }
    ...
    protected void send(Payload payload, String endpointId) {
        sendOne(payload, endpointId);
    }

    private void sendOne(Payload payload, String endpointId) {
        mConnectionsClient
   ...
   * @param payload The data you want to send.
   */
  protected void sendAll(Payload payload) {
    sendAll(payload, mEstablishedConnections.keySet());
  }

  private void sendAll(Payload payload, Set<String> endpoints) {
    mConnectionsClient
